...
<p align="center">
  <!-- 📷 图片 #3（最关键的一张）：Fusion 螺纹对话框，「螺纹类型」下拉框展开，
       高亮显示「Camera Lens and Filter Threads」。详见 docs/images/README.md -->
  <img src="docs/images/thread-type-dropdown.png" alt="安装后「Camera Lens and Filter Threads」出现在 Fusion 360 螺纹类型下拉框中" width="720">
  <br><em>安装后，<strong>Camera Lens and Filter Threads</strong> 会直接出现在螺纹命令的「螺纹类型」里。</em>
</p>
...
两种螺距都有存在的理由：**同一个直径数字，可能是滤镜螺纹（0.75 mm），也可能是镜头卡口（1.0 mm）**——靠螺距区分。

**速记**
- **0.75 mm** → 镜头**前面那一圈螺纹**（拧 UV/ND/CPL 滤镜、镜头盖、遮光罩、升降环）。这是口径 ≤86 mm 滤镜螺纹的摄影行业标准。
- **1.0 mm** → (a) **大口径滤镜**（95、105、112、120、127 mm）；(b) **螺丝镜头卡口**，如 `M42×1`（单反）、`M39×1`（徕卡螺口旁轴）。

| 你要做… | 选 | 例子 |
|---|---|---|
| 螺丝滤镜、镜头盖、遮光罩、滤镜升降环（≤86 mm） | **0.75 mm** | 49、52、67、77 mm 滤镜螺纹 |
| 大口径滤镜配件（≥95 mm） | **1.0 mm** | 95、105、112、120、127 mm |
| 转接老镜头螺丝卡口的转接环 | **1.0 mm** | `M42×1` 卡口、`M39×1` 徕卡螺口 |
| 不确定 / 通用滤镜件 | 默认 **0.75 mm** | 稳妥选择 |

**容易混的尺寸：**
- **M39**：`M39×0.75` = 39 mm 滤镜螺纹；`M39×1.0` = 徕卡螺口（旁轴老镜头）。同数字不同用途。
- **M40 / 40.5**：滤镜螺纹 → `0.75 mm`。
- **M42**：`M42×0.75` = 42 mm 滤镜螺纹；`M42×1.0` = 著名单反螺丝卡口（转接环用 1.0）。
- **M43**：滤镜螺纹 → `0.75 mm`。

在 Fusion 360 中，只要在「规格 (Designation)」下拉里选对应项即可——下方截图展示了两种螺距并列的样貌。
...
<p align="center">
  <img src="docs/images/designation-list.png" alt="Fusion 360 规格下拉框，M46x0.75 与 M46x1.0 并列显示" width="560">
  <br><em>「规格 (Designation)」下拉框为每个尺寸列出两种螺距——如 <code>M46x0.75</code>（滤镜）与 <code>M46x1.0</code>（卡口）。</em>
</p>
...
**第三步 — 验证：** 打开螺纹命令，在「螺纹类型」下拉框里找到 `Camera Lens and Filter Threads`。完成 ✅
...
**第三步 — 验证：** 打开螺纹命令 →「螺纹类型」下拉框里已出现 `Camera Lens and Filter Threads`。完成 ✅
...
然后设置 **螺纹类型 → `Camera Lens and Filter Threads`**。
...
<p align="center">
  <img src="docs/images/fusion-create-thread.png" alt="Fusion 360 创建螺纹对话框，已选择 Camera Lens and Filter Threads" width="820">
  <br><em>「螺纹 (Thread)」对话框，已选中 <strong>Camera Lens and Filter Threads</strong>——选择尺寸和规格后点确定即可。</em>
</p>

<p align="center">
  <img src="docs/images/result-part.png" alt="应用建模螺纹后的零件效果" width="820">
  <br><em>最终效果：零件上生成了完整的镜头螺纹建模，可直接用于 3D 打印或机加工。</em>
</p>
...
| 现象 | 解决办法 |
|------|----------|
| 看不到 `Camera Lens and Filter Threads` | 是否**完全重启**了 Fusion 360？自定义螺纹在启动时加载。 |
